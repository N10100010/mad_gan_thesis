...
from datasets.cifar import cmadgan_dataset_func as dataset_func
...
def save_generated_images(
    ...
    for i in range(dim[0]):  # Loop through generators
        for j in range(dim[1]):  # Loop through examples (digits 0-9)
            ...
            img = tf.reshape(img, (32, 32, 3))  # Reshape if necessary
    ...
    plt.suptitle(f"CMAD-GAN CIFAR Generated Images - Epoch {epoch+1}", fontsize=14)
    save_path = os.path.join(output_dir, f"cifar_epoch_{epoch+1:04d}.png")
    plt.savefig(save_path)
    print(f"Saved generated image grid to {save_path}")
    plt.close(fig)  # Close the figure to free memory


class CIFAR_CMADGAN_Experiment(BaseMADGANExperiment):
